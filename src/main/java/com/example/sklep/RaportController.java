...
public class RaportController {
    ...
    @FXML
    private TextField filterPhraseTextField; // Pole tekstowe dla frazy filtrującej
    ...
    private void initialize() {
        ...
        reportTypeChoiceBox.getItems().addAll("Magazyn", "Pracownicy", "Użytkownicy");
        ...
        reportTypeChoiceBox.getSelectionModel().selectedItemProperty().addListener((observable, oldValue, newValue) -> {
            if ("Magazyn".equals(newValue)) {
                filterTypeChoiceBox.getItems().setAll("Brak", "Zasoby");
                filterTypeChoiceBox.setValue("Brak");
            } else if ("Pracownicy".equals(newValue)) {
                filterTypeChoiceBox.getItems().setAll("Brak", "Imie", "Nazwisko");
                filterTypeChoiceBox.setValue("Brak");
            } else if ("Użytkownicy".equals(newValue)) {
                filterTypeChoiceBox.getItems().setAll("Brak", "ID Roli");
                filterTypeChoiceBox.setValue("Brak");
            }
        });
    ...
    private void handleGenerateReport() {
        ...
        String filterPhrase = filterPhraseTextField.getText(); // Inicjalizacja frazy filtra (może być pusta)
    ...
    private Integer getFilterType(String reportType, String filterType) {
        ...
        } else if ("Pracownicy".equals(reportType)) {
            switch (filterType) {
                case "Brak":
                    return 1;
                case "Imie":
